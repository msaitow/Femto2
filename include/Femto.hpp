...
namespace Femto{
  ...
  ////////////////////////////////////////////////
  const SQcont<std::string> UNITY;
  const SQcont<std::string> ECAS("Ecas");
  ////////////////////////////////////////////////
  ...
  enum char_state {core  =0, act  =1, virt  =2, aux=3, 
		   ...
		   core_b=7, act_b=8, virt_b=9, gen=10,
                   ao    =11};  
  ...
  Symmetry t2_symm(); 
  Symmetry u4_symm();
  Symmetry un_symm(int n); // Returns symmetry for the n-rank tensor
  ...
  // Interaction parser
  SQcont<char_state> GenerateInteractions(std::string input);
  ...
  // *********************************************************
  // Definition of name of soGen
  // *********************************************************
  inline std::string soGen_name()
  { return "G"; }
  ...
  // *********************************************************
  // Print predefined names of the operators and tensors
  // *********************************************************
  inline void printOps()
  {
    std::cout << " >> Spin-free    generator    << " << sfGen_name()  << std::endl;
    std::cout << " >> Spin-orbital generator    << " << soGen_name()  << std::endl;
    std::cout << " >> Reduced-Density Matrix    << " << RDM_name()    << std::endl;
    std::cout << " >> Kronecker's delta         << " << kDelta_name() << std::endl;
    std::cout << " >> Creation operator         << " << aCre_name()   << std::endl;
    std::cout << " >> Destruction operator      << " << aDes_name()   << std::endl;
  }
  ...
  // Whether name is equal to that of sfGen
  // *********************************************************
  inline bool is_soGen(const std::string name)
  { 
    std::string num(name.substr(1));
    if(name.at(0) == soGen_name()[0] && atoi(num.c_str())) return true;
    else                                                   return false;
  }
