...
namespace Femto { namespace Core {
  ...
  struct PtrGreat { bool operator()(const SharedIndex i, const SharedIndex j) const 
    { return (*i) > (*j); } };
  ...
  // 
  // *********************************************************
  SQtensor::SQtensor(const string &name, const SharedIndices indices, 
  ...
  bool SQtensor::operator==(const SQtensor &obj) const{
    ...
    SharedIndices Is; Is.reserve(indices_.size()); 
    SharedIndices Js; Js.reserve(obj.indices_.size());
    for(size_t num_i = 0;num_i < indices_.size();++num_i) 
      if(!indices_[num_i]->get_isSummed()) Is <= indices_[num_i]; 
    for(size_t num_j = 0;num_j < obj.indices_.size();++num_j) 
      if(!obj.indices_[num_j]->get_isSummed()) Js <= obj.indices_[num_j]; 
...
//		obj.sortIndices();
		for(size_t num_i = 0;num_i < indices_.size();++num_i){
                  if      (*indices_[num_i]  < *obj.indices_[num_i]) retval = true;
		  else if (*indices_[num_i]  > *obj.indices_[num_i]) retval = false;
		  else if (*indices_[num_i] == *obj.indices_[num_i]) retval = false;
		} // End num_i
...
//--OLD--   // *********************************************************
//--OLD--   //  *NOTE* Make sure this order is not always unique if 
//--OLD--   //         there are several similar tensors.
//--OLD--   // *********************************************************
//--OLD--   bool SQtensor::operator<(const SQtensor &obj) const
//--OLD--   {
//--OLD--     bool retval;
//--OLD--     if      (name_  < obj.name_) retval = true;
//--OLD--     else if (name_  > obj.name_) retval = false;
//--OLD--     else if (name_ == obj.name_){
//--OLD--       //if     (sign_ < obj.sign_) retval = true;
//--OLD--       //else if(sign_ > obj.sign_) retval = false;
//--OLD--       //else if(sign_ == obj.sign_){
//--OLD--       if     (not_  < obj.not_) retval = true;
//--OLD--       else if(not_  > obj.not_) retval = false;
//--OLD--       else if(not_ == obj.not_){ 
//--OLD--         if     (isCommutable_  < obj.isCommutable_) retval = true;
//--OLD--         else if(isCommutable_  > obj.isCommutable_) retval = false;
//--OLD--         else if(isCommutable_ == obj.isCommutable_){
//--OLD--           if      (permutations_  < obj.permutations_) retval = true;
//--OLD--           else if (permutations_  > obj.permutations_) retval = false;
//--OLD--           else if (permutations_ == obj.permutations_){
//--OLD--             if      (factors_  < obj.factors_) retval = true;
//--OLD--             else if (factors_  > obj.factors_) retval = false;
//--OLD--             else if (factors_ == obj.factors_){
//--OLD--               if      (indices_.size()  < obj.indices_.size()) retval = true;
//--OLD--               else if (indices_.size()  > obj.indices_.size()) retval = false;
//--OLD--               else if (indices_.size() == obj.indices_.size()){
//--OLD-- 
//--OLD--                 // Compare the numbers of                 
//--OLD--                 SharedIndices my_indices;  my_indices.reserve(indices_.size());               
//--OLD-- 		for(auto i = indices_.begin();i != indices_.end();++i) if(!(*i)->get_isSummed()) my_indices <= *i;               
//--OLD-- 		sort(my_indices.begin(), my_indices.end());
//--OLD--                 SharedIndices obj_indices; obj_indices.reserve(indices_.size());               
//--OLD-- 		for(auto i = obj.indices_.begin();i != obj.indices_.end();++i) if(!(*i)->get_isSummed()) obj_indices <= *i;               
//--OLD-- 		sort(obj_indices.begin(), obj_indices.end());
//--OLD-- 
//--OLD-- 		if     (my_indices  < obj_indices) retval = true;
//--OLD-- 		else if(my_indices  > obj_indices) retval = false;
//--OLD-- 		else if(my_indices == obj_indices){
//--OLD-- 
//--OLD-- 		  // Count the scores
//--OLD-- 		  int ncore_(0), obj_ncore_(0);
//--OLD-- 		  int nact_ (0), obj_nact_ (0);
//--OLD-- 		  int nvir_ (0), obj_nvir_ (0);
//--OLD-- 		  for(size_t i = 0;i < indices_.size();++i){
//--OLD-- 		    if      (    indices_[i]->get_char() == 0) ++ncore_;
//--OLD-- 		    else if (    indices_[i]->get_char() == 1) ++nact_;
//--OLD-- 		    else if (    indices_[i]->get_char() == 2) ++nvir_;
//--OLD-- 		    if      (obj.indices_[i]->get_char() == 0) ++obj_ncore_;
//--OLD-- 		    else if (obj.indices_[i]->get_char() == 1) ++obj_nact_;
//--OLD-- 		    else if (obj.indices_[i]->get_char() == 2) ++obj_nvir_;
//--OLD-- 		  } // End i
//--OLD-- 		  
//--OLD-- 		  if      (ncore_  < obj_ncore_) retval = true;
//--OLD-- 		  else if (ncore_  > obj_ncore_) retval = false;
//--OLD-- 		  else if (ncore_ == obj_ncore_) {
//--OLD-- 		    if      (nact_   < obj_nact_ ) retval = true;
//--OLD-- 		    else if (nact_   > obj_nact_ ) retval = false;
//--OLD-- 		    else if (nact_  == obj_nact_ ) {
//--OLD-- 		      if      (nvir_   < obj_nvir_ ) retval = true;
//--OLD-- 		      else if (nvir_   > obj_nvir_ ) retval = false;
//--OLD-- 		      else if (nvir_  == obj_nvir_ ) {
//--OLD-- 			// Count number of non-dummy indices
//--OLD-- 			int count_ = 0, obj_count_ = 0;
//--OLD-- 			for(size_t i = 0;i < indices_.size();++i){
//--OLD-- 			  if(!    indices_[i]->get_isSummed()) ++count_;
//--OLD-- 			  if(!obj.indices_[i]->get_isSummed()) ++obj_count_;
//--OLD-- 			} // End i
//--OLD-- 			if      (count_  < obj_count_) retval = true;
//--OLD-- 			else if (count_  > obj_count_) retval = false;
//--OLD-- 			else if (count_ == obj_count_) retval = false;
//--OLD-- 		      } // End if(ncore_)
//--OLD-- 		    } // End if(nact_ )
//--OLD-- 		  } // End if(nvirt_)
//--OLD-- 		} // End if(my_indices)
//--OLD--   	      } // End if(indices_.size())
//--OLD--   	    } // End if(factors_)
//--OLD--   	  } // End if(permutations_)
//--OLD--         } // End if(isCommutable_)
//--OLD--       } // End if(not_)
//--OLD-- 	//}
//--OLD--     } // End if(name_)
//--OLD-- 
...
//*OBSOLETE*   }
  ...
  // 
  // *********************************************************
  SQcont<SharedIndex> SQtensor::get_indices() const
  { return indices_; }
  ...
  // 
  // *********************************************************
  void SQtensor::put_indices(size_t i, SharedIndex v)
  { 
    auto it = indices_.begin(); it += i;
  ...
  {
    std::string IndLabel;
    SQcont<SharedIndex> indices(s.get_indices());
...
#ifdef _ABORT
      if(!unique) {
	cout << "No unique winner produced when sorting the indices" << endl;
        abort();
      } // End if
#endif
    } // End if

    SQcont<SharedIndex> newIndices;
    for(size_t i = 0;i < tuples[0].size();++i)
      newIndices <= indices_[tuples[0][i]];
...
//*BUG*     if(it != indices_.end()) retval = true;
  ...
  void SQtensor::rotateIndices(const size_t i)
  {
    ...
    for(size_t j = 0;j < permutations_[i].size();++j){
      newIndices <= indices_[permutations_[i][j]];
    } // End j
    indices_ = newIndices;
  ...
  void SQtensor::convertD2M()
  {
    ...
    Ivector temp_i; 
    SQcont<SharedIndex> newIndices; newIndices.reserve(2*order);
    for(size_t i = 0;i < order;++i){
      newIndices <= indices_[i      ];
      newIndices <= indices_[i+order];
      temp_i.push_back(i      );
...
//    cout << endl;
    ...
    auto p = permutations_.begin();
    for(;p != permutations_.end();++p){
      vector<int> q;
      newPerm.push_back(q);
      for(size_t i = 0;i < order;++i){
	auto it = newPerm.end(); --it;
...
//*OLD*     abort();
    ...
    SQcont<SharedIndex> newIndices; newIndices.reserve(2*order);
    for(size_t i = 0;i < order;++i){
      newIndices <= indices_[2*i  ];
      temp_i.push_back(2*i  );
    } // End i
    for(size_t i = 0;i < order;++i){
      newIndices <= indices_[2*i+1];
  ...
  {
    if(ta.get_name() != tb.get_name()) return false;
    const SharedIndices ind_a(ta.get_indices());
    const SharedIndices ind_b(tb.get_indices());
    if(ind_a.size() != ind_b.size()) return false;

    for(size_t num = 0;num < ind_a.size();++num) if(*ind_a[num] != *ind_b[num]) return false;
  ...
  // Kronecker's delta class
  // *********************************************************
  kDelta::kDelta(const SQcont<SharedIndex> indices)
  { 
  ...
  sfGen::sfGen(const SQcont<SharedIndex> indices)
  { 
    name_         = sfGen_name();
    indices_      = indices;
    isCommutable_ = false;
...
//*OLD*   {}
  ...
  // *********************************************************
  RDM::RDM(const SQcont<SharedIndex> indices)
  { 
    name_         = RDM_name();
    indices_      = indices;
    isCommutable_ = true;
...
//*DEBUG*     }
  ...
  // *********************************************************
  aCre::aCre(SharedIndex index)
  { 
    SQcont<SharedIndex> indices;
    indices       <= index;
    name_         = aCre_name();
    indices_      = indices;
    isCommutable_ = false;
