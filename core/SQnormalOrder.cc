...
namespace Femto { namespace Core { 
  ...
  // ****************************************************************
  void normalOrder(SQcont<SQterm> *inTerms, const bool OF_flag)
  {
    pair<bool, bool> so_sf(false, false); // <so-base, sf-base>
    for(size_t num_t = 0;num_t < inTerms->at(0).get_tensors().size();++num_t)
      for(size_t num_i = 0;num_i < inTerms->at(0).get_tensors()[num_t].get_indices().size();++num_i)
	if(inTerms->at(0).get_tensors()[num_t].get_indices()[num_i]->get_char() != gen){
	  if(isSD(*(inTerms->at(0).get_tensors()[num_t].get_indices()[num_i])))
	    so_sf.first = true;
	  else if(isSF(*(inTerms->at(0).get_tensors()[num_t].get_indices()[num_i])))
	    so_sf.second = true;
	} // End if
...
//*NOT YET*     else if( so_sf.first &&  so_sf.second) normalOrder_so_sf(inTerms, OF_flag);
  ...
  // ----------------------------------------------------------------
  void normalOrder_sf(SQcont<SQterm> *inTerms, const bool OF_flag)
  {
...
//          inTerms->at(0).get_summedBody()[num]->get_char() != virt){
      if(!isSF(*inTerms->at(0).get_summedBody()[num])){
    ...
    SQtensors inTensors((*inTerms)[0].get_tensors());
    ...
    cv_pair upp_cv(0, 0); // <num core, num virt> for upper side
    cv_pair low_cv(0, 0); // <num core, num virt> for lower side
    ...
    int totnum(0);
    Ivector orders;
    for(auto gen = sfGen_list.begin();gen != sfGen_list.end();++gen)
      orders.push_back((gen->get_indices().size())/2);
    IIvector init_orders; init_orders.push_back(orders);
    IIvector out_orders;
    while(m > 1){
      out_orders.clear();
      for(auto o = init_orders.begin();o != init_orders.end();++o){
        auto o0(o->begin());
        auto o2(o->end()); --o2;
        auto o1(o2);       --o1;
        Ivector except_last_two;
        ...
        for(int nc = 0;nc < min(*o1,*o2)+1;++nc){
          ...
          Ivector o_list(except_last_two);
    ...
    SQterms iter_terms; iter_terms.reserve(totnum);
   ...
   size_t Count(0);
   for(auto t = iter_terms.begin();t != iter_terms.end();++t){
     if(t->get_isInCanonical()) break;
     else ++Count;    
     
     // Make a list of the term's tensors that excludes the last two excitation operators
     SQtensors tensors_except_last_two;
     SQtensors t_temp = t->get_tensors();
     auto it    = t_temp.begin();
     auto it_m1 = t_temp.end(); 
     auto it_m2 = t_temp.end(); 
     --it_m1;
     --it_m2; --it_m2;
       ...
       for(auto perm = perms.begin();perm != perms.end();++perm){
         for(auto tup1 = tups1.begin();tup1 != tups1.end();++tup1){
           for(auto tup2 = tups2.begin();tup2 != tups2.end();++tup2){
             ...
             SharedIndices indexList; indexList.reserve(2*newOrder);
             SharedIndex q = NULL;
             ...
             SharedIndices e1_indices(e1.get_indices());
             SharedIndices e2_indices(e2.get_indices());
	       ...
	       auto i_index = find(conPairs[0].begin(), conPairs[0].end(), i);
               ...
               if(i_index != conPairs[0].end()){
                 ...
                 SharedIndex ind1(e1_indices[i1]); 
                 SharedIndex ind2(e2_indices[i2]); 

                 SharedIndices ind_delta;
                 ind_delta.push_back(ind1);
                 ind_delta.push_back(ind2);
                 if((ind_delta[0]->get_char()!=ind_delta[1]->get_char()) && (ind_delta[0]->get_char()!=Femto::gen && ind_delta[1]->get_char()!=Femto::gen)) isDead = true;
	       ...
	       auto i_index = find(conPairs[1].begin(), conPairs[1].end(), i);
	       ...
	       if(i_index==conPairs[1].end()){
		     ...
		     /////////////////////////// core=0       ---------------------- gen=10        /////////////////////////////////////////////////////////////////
                     else if(((*I)->get_char()==Femto::core || ((*I)->get_char()==Femto::gen)) && ((size_t)(I-ind.begin()) <  (ten_E->get_indices().size())/2)) 
                       ++numCore.first;
                     else if(((*I)->get_char()==Femto::core || ((*I)->get_char()==Femto::gen)) && ((size_t)(I-ind.begin()) >= (ten_E->get_indices().size())/2)) 
                       ++numCore.second;
                     if(isVanished) break;
