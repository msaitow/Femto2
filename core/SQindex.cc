...
namespace Femto { namespace Core {
  ...
  { 
    ...
    //if((int)charactar_ < 0 || (int)charactar_ > 11 ){
    if(!isValid(*this)){
...
//        (int)charactar_!=7  && (int)charactar_!=8  && (int)charactar_!=9  && 
//        (int)charactar_!=10 && (int)charactar_!=11){
    if(!isValid(*this) || charactar_ == aux){
      cout << "Incorrect orbital type is specified ... " << endl;
...
//*ALTERNATIVE*   }
  ...
  operator<<(std::ostream &os, const SQindex &i)
  {
    ...
    //*-- Generic index *--//
    else if (i.charactar_ == gen   ) IndType = "\"generic\""; 
    //*-- AO basis index --*//
    else if (i.charactar_ == ao    ) IndType = "\"ao basis\""; 
    //os << "@[" << i.index_ << ", " << IndType << "]";
    os << boost::format("@[%4s, %10s]") % i.index_ % IndType;
    return os;
  }
